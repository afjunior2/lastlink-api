using LastLinkApi.Application.Commands;
using LastLinkApi.Application.Handlers;
using LastLinkApi.Domain.Entities;
using LastLinkApi.Domain.Repositories;
using LastLinkApi.Domain.ValueObjects;
using Moq;
using Xunit;

namespace LastLinkApi.Tests.Application
{
    public class RejectAdvanceRequestHandlerTests
    {
        [Fact]
        public async Task Deve_Rejeitar_Solicitacao()
        {
            // Arrange
            var request = new AdvanceRequest("user-123", 2000);
            request.SetId(2);

            var repoMock = new Mock<IAdvanceRequestRepository>();
            repoMock.Setup(r => r.GetByIdAsync(2)).ReturnsAsync(request);
            repoMock.Setup(r => r.UpdateAsync(It.IsAny<AdvanceRequest>()))
                .Returns(Task.CompletedTask);

            var handler = new RejectAdvanceRequestHandler(repoMock.Object);
            var command = new RejectAdvanceRequestCommand(2);

            // Act
            var result = await handler.Handle(command, CancellationToken.None);

            // Assert
            Assert.Equal(RequestStatus.Rejected, result.Status);
            repoMock.Verify(r => r.UpdateAsync(It.Is<AdvanceRequest>(a => a.Id == 2)), Times.Once);
        }
    }
}